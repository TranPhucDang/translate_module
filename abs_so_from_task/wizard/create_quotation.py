...
class CreateQuotation(models.TransientModel):
    ...
    task_order_date = fields.Date(string='Order Date', required=True, help='Date on which the sale order is confirmed')
        ...
        for record in self:
            if self.env.context.get('active_model') == 'project.task':
                ...
                if store_id:
                    ...
                    so_quotation = {'task_order_date':store_task_order_date,'partner_id':task_partner_id,'source_project_task_id':store_id}
